...
paraSet = {'Select'};
Datasets = {'NLPR','STERE','SSD','LFSD','DES','NJUDS','DUT','SIP'};
Models = {'Ours','Ours+'};
DataPath = '../TestingSet/';
...
for p = 1:1
    ...
    SalMapPath = ['./']; %Put model results in this folder.

    %Results setting
    ResDir = [SalMapPath 'Evaluation_Curves/'];
    if ~exist(ResDir,'dir')
        mkdir(ResDir);
    end
    ...
    for d = 1:length(Datasets)
        ...
        for m = 1:modelNum
            ...
            imgFiles = dir([salPath '*.*g']);

            if ~length(imgFiles)
                imgFiles = dir([salPath '*.jpg']);
            end

            if ~length(imgFiles)
                disp(' Don''t find saliency maps');
            end
            ...
            for i = 1:imgNUM
                ...
                fprintf('Evaluating(%d %s Dataset,%s Model):%d/%d\n', p, dataset, model, i,imgNUM);
                name =  imgFiles(i).name;

                %load gt
                gt = imread([gtPath name(1:end-3) 'png']);
            ...
            save([ResPath dataset '-' model],'Smeasure', 'mae', 'column_Pr', 'column_Rec', 'column_F', 'adpFm', 'meanFm', 'maxFm', 'column_E', 'adpEm', 'meanEm', 'maxEm');
